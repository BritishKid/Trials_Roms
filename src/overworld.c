...
static bool16 ShouldLegendaryMusicPlayAtLocation(struct WarpData *warp)
{
    ...
    if (warp->mapGroup == 0)
    {
        switch (warp->mapNum)
        {
        // case MAP_NUM(BREAKWATER_PORT): //these dont work cause they are not in mapgroup 0 and have been moved
        // case MAP_NUM(HEMLOCK_VILLAGE):
...
{
    // if (VarGet(VAR_HEMLOCK_VILLAGE_STATE) == 0)
    //     return FALSE;
    // else if (VarGet(VAR_HEMLOCK_VILLAGE_STATE) > 2)
    //     return FALSE;
...
u16 GetWarpDestinationMusic(void)
{
    ...
    {
        if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(FALL_HARBOUR)
         && gSaveBlock1Ptr->location.mapNum == MAP_NUM(FALL_HARBOUR))
            return MUS_ROUTE110;
        else
            return MUS_ROUTE119;
    }
