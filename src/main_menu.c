    ...
    if (!gPaletteFade.active)
    {
        ...
        switch (action)
        {
            ...
            default:
                ...
                gTasks[taskId].func = Task_NewGameBirchSpeech_Init; //new game starts?
                // gTasks[taskId].func = Task_NewGameBirchSpeech_Cleanup; //new game starts?
                break;
...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_INKAY);
...
{
    return CreateMonPicSprite_Affine(SPECIES_INKAY, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
